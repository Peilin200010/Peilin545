...
class ByDistri:
    ...
    def portfolio_VaR_ES_t(self, prices: pd.DataFrame, P: dict, alpha: float = 0.05,
        ...
        for col in returns_P.columns:
            free, mu, sigma = sps.t.fit(returns_P[col])
            U[col] = returns_P[col].apply(lambda x: sps.t.cdf(x, free, mu, sigma))
            frees.append(free)
            mus.append(mu)
            sigmas.append(sigma)

        cor = sps.spearmanr(U)[0]
        N = PCA().simulation(cor)

        # transform back
        U_sim = sps.norm.cdf(N)
        sim_returns = U_sim.copy()
        for i in range(U_sim.shape[1]):
            sim_returns[:, i] = sps.t.ppf(U_sim[:, i], frees[i], loc=mus[i], scale=sigmas[i])
